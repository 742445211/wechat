...
<!--/_menu 作为公共模版分离出去-->
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="admin_add('添加用户','/typeadd','510','300')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加类型</a></span> <span class="r">共有分类数：<strong>{{$num}}</strong> 个</span> </div>
			<div class="mt-20">
     	...
     	for(m=1;m<2;m++){
          ...
          tabs.rows[i].cells[m].ondblclick = function(){
              ...
              inp[0].onblur = function(){
				...
				v = this.value;
				$.get('/typeedit',{id:id,v:v},function(data){
					if(data == 1){
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.get('/moneystatus',{id:id},function(data){
			if(data == 0){
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.get('/moneydel',{id:id},function(data){
			if(data == 1){
