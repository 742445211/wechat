...
<!--/_menu 作为公共模版分离出去-->
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="javascript:;" onclick="admin_add('添加热结算周期','/timesadd','510','300')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加结算周期</a>&nbsp;&nbsp;&nbsp;Tip:双击内容即可进行修改操作!</span> <span class="r">共有数据：<strong>{{$num}}</strong> 条</span> </div>
			<div class="mt-20">
     	...
     	for(m=1;m<2;m++){
          ...
          tabs.rows[i].cells[m].ondblclick = function(){
              ...
              inp[0].onblur = function(){
				...
				v = this.value;
				$.get('/timesedit',{id:id,v:v},function(data){
					if(data == 1){
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		$.get('/timesstatus',{id:id},function(data){
			if(data == 0){
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.get('/timesdel',{id:id},function(data){
			if(data == 1){
...
@endsection
@section('title','结算周期管理')
@section('banner','结算周期管理')