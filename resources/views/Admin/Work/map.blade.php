...
<link rel="stylesheet" type="text/css" href="/index/lib/Hui-iconfont/1.0.8/iconfont.css" />
    ...
    <script type="text/javascript"
            src="http://webapi.amap.com/maps?v=1.3&key=d5e372073f391117a790cf5cdc16d6f4&plugin=AMap.Autocomplete"></script>
...
<div id="myPageTop">
    ...
    <form method="get" action="/worklist/create" class="form form-horizontal" enctype="multipart/form-data">
        <div class="row cl">
...
<script type="text/javascript">
    ...
    var clickEventListener = map.on('click', function(e) {
		...
		$('#map').val(address);
		$.get('/getadd',{location:address},function(data){
    ...
    $('#address').on('input',function(){
        ...
        $.get('/getadd',{address:address},function(data){
            if(data.geocodes[0].formatted_address){
                document.getElementById("lnglat").value = data.geocodes[0].location;
                $('#map').val(data.geocodes[0].location)
                $('#address').val(data.geocodes[0].formatted_address);   // 详细地址
                //市加区
                $('#addresslite').val(data.geocodes[0].city+'/'+data.geocodes[0].district);
            }else{
                $('#address').val('获取失败!');
            }
        })
